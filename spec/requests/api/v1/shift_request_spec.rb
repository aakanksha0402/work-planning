...
RSpec.describe "Shifts", type: :request do
  ...
  let!(:shift) { Shift.create(shift_name: 'Shift A', work_date: '22/12/2022', worker: worker) }
  ...
  describe "CREATE: POST /shifts" do
    it "returns http success" do
      post "/api/v1/shifts", params: {shift: {shift_name: "Shift A", work_date: "23/12/2022", worker_id: worker.id}}
      response_body = JSON.parse(response.body)
      expect(response_body['shift_name']).to eq('Shift A')
      expect(response_body['worker']).not_to be nil
      expect(response.status).to eq(200)
    end
    ...
    context 'when worker does not exist' do
      it 'returns http error' do
        post "/api/v1/shifts", params: {shift: {shift_name: "Shift A", work_date: "23/12/2022"}}
        response_body = JSON.parse(response.body)
    ...
    context 'when a shift is already present' do
      it "returns http error" do
        post "/api/v1/shifts", params: {shift: {shift_name: "Shift B", work_date: "22/12/2022", worker_id: worker.id}}
        response_body = JSON.parse(response.body)
    ...
    context 'when shift_name or work_date is not present' do
      it "returns http error" do
        ...
        response_body = JSON.parse(response.body)
        expect(response_body['error']).to eq(["Shift name can't be blank", "Work date can't be blank"])
        expect(response.status).to eq(422)
      end
    end
  end
  ...
  describe "UPDATE: PUT /shifts/:id" do
    it "returns http success" do
      put "/api/v1/shifts/#{shift.id}", params: {shift: {shift_name: "Shift B"}}
      response_body = JSON.parse(response.body)
      expect(response_body['shift_name']).to eq('Shift B')
      expect(response.status).to eq(200)
    end
  end
